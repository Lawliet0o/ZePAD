    ...
    if encoder != None:
        with torch.no_grad():
            for counter, (x_batch, y_batch) in enumerate(tqdm(test_loader)):
                ...
                logits = classifier(downstream_input)
        ...
        with torch.no_grad():
            for counter, (x_batch, y_batch) in enumerate(tqdm(test_loader)):
                x_batch = x_batch.cuda()
                y_batch = y_batch.cuda()
                logits = classifier(normalzie(args, x_batch))
                top1, top5 = accuracy(logits, y_batch, topk=(1, 5))
                top1_accuracy += top1[0]
                top5_accuracy += top5[0]
...
def test_finetuneVer(args, model, test_loader):
    top1_accuracy = 0
    top5_accuracy = 0
    ...
    if encoder != None:
        with torch.no_grad():
            for counter, (x_batch, y_batch) in enumerate(tqdm(test_loader)):
                ...
                c_logits = classifier(c_downstream_input)
                p_logits = classifier(p_downstream_input)
        ...
        with torch.no_grad():
            for counter, (x_batch, y_batch) in enumerate(tqdm(test_loader)):
                ...
                per_x_batch = per_x_batch.cuda()
                c_logits = classifier(normalzie(args, clean_x_batch))
                p_logits = classifier(normalzie(args, per_x_batch))
