...
def arg_parse():
    parser = argparse.ArgumentParser(description='Train downstream models using of the dual encoder')
    ...
    parser.add_argument('--ssl_method', default='simclr', choices=['simclr', 'byol', 'barlow_twins', 'deepclusterv2', 'dino', 'mocov3', 'mocov2plus', 'nnclr', 'ressl', 'simsiam', 'supcon', 'swav', 'vibcreg', 'vicreg', 'wmse','VIT-dino'])
...
def classify(args, encoder):
    ...
    # save uap result
    uap_save_path = os.path.join('./clean_downstream', str(args.pre_dataset), str(args.ssl_method), str(args.dataset))
    ...
    for epoch in range(args.epochs):
        ...
        torch.save(F.state_dict(), '{}/{}'.format(uap_save_path,  str(args.ssl_method) + '_' +str(args.pre_dataset) + '_' + str(args.dataset) + '_' + str(
...
def main():
    ...
    # Logging
    log_save_path = os.path.join('./output', str(args.pre_dataset), 'log', 'down_test', "clean_model", str(args.ssl_method), str(args.dataset))
    ...
    print('Day: %s, Target encoder:%s, Downstream task:%s'% (now_time, args.ssl_method,  args.dataset))
