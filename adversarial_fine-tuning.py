...
from utils.data_loder import load_data, normalzie
from utils.predict import make_print_to_file, test_finetuneVer, rob_test, accuracy
from models.linear import NonLinearClassifier
from utils.gr import genetic_regularization
from pathlib import Path

def arg_parse():
    parser = argparse.ArgumentParser(description='adversarial fine-tuning')
    ...
    parser.add_argument('--ssl_method', default='wmse', choices=['simclr', 'byol', 'dino', 'mocov3', 'mocov2plus',
                                 'nnclr', 'ressl', 'swav', 'vibcreg', 'wmse','supcon','VIT-dino'])
...
def classify(args, encoder):
    ...
    uap_save_path_e = os.path.join('./finetuned', str(args.pre_dataset),  str(args.ssl_method), str(args.dataset), 'encoder')
    uap_save_path_f = os.path.join('./finetuned', str(args.pre_dataset),  str(args.ssl_method), str(args.dataset), 'f')
    ...
    if args.ssl_method == 'VIT-dino':
        F = NonLinearClassifier(feat_dim=192, num_classes=num_classes)
    else:
    ...
    for epoch in range(args.epochs):
        ...
        clean_acc_t1, clean_acc_t5 = test_finetuneVer(args, model, test_loader)
        ...
        if args.save == 'True':
            ...
            if adv_acc_t1 > best_adv_acc_t1:
                ...
                # save encoder
                torch.save(encoder,
                           '{}/{}'.format(uap_save_path_e, str(args.ssl_method) + '_' + str(args.pre_dataset) + '_' + str(
                               args.dataset) + '_last' + '.pth'))
                # save F
                torch.save(F,
                           '{}/{}'.format(uap_save_path_f, str(args.ssl_method) + '_' + str(args.pre_dataset) + '_' + str(
                               args.dataset) + '_last' + '.pth'))
...
def main():
    ...
    # Logging
    log_save_path = os.path.join('./finetune_output', str(args.pre_dataset), 'log', 'down_test', "aft_model", str(args.ssl_method),
                                 str(args.dataset))
    ...
    print('Day: %s, Target encoder:%s, training dataset:%s' % (now_time, args.ssl_method, args.dataset))
    print("######################################  aft begin ######################################")
