...
def load_data(data, bs):
    ...
    if data == 'cifar10':
        train_dataset = datasets.CIFAR10('../Dataset/data/cifar10/', train=True, download=True, transform=transform_)
        test_dataset = datasets.CIFAR10('../Dataset/data/cifar10/', train=False, download=True, transform=transform_)
        ...
        # /before11.30
        # 11.30
        train_dataset = datasets.GTSRB('../Dataset/data/GTSRB/', split="train", download=True, transform = transform_)
        test_dataset = datasets.GTSRB('../Dataset/data/GTSRB', split="test", download=True, transform=transform_)
        # /11.30

    elif data == 'imagenet':
        train_dataset = datasets.ImageFolder('../Dataset/data/imagenet100/imagenet100', transform = transform_)
        train_dataset, test_dataset = torch.utils.data.random_split(train_dataset, [110000, len(train_dataset)-110000])
