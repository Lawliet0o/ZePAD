...
from utils.load_model import load_finetuned_encoder_F,load_encoder
from models.linear import NonLinearClassifier


def arg_parse():
    parser = argparse.ArgumentParser(description='Train downstream models using of the single encoder')
    ...
    parser.add_argument('--ft_dataset',type=str,default='cifar10')
...
def classify(args, encoder):
    ...
    # save uap result
    uap_save_path = os.path.join('./aft_downstream_se', str(args.pre_dataset), str(args.ssl_method),str(args.ft_dataset) ,str(args.dataset))
...
def main():
    ...
    # Logging
    log_save_path = os.path.join('./output', str(args.pre_dataset), 'log', 'down_train', "clean_model", str(args.ssl_method), str(args.dataset))
    ...
    # model = dual_encoder(args)
    model ,_ = load_finetuned_encoder_F(args)
