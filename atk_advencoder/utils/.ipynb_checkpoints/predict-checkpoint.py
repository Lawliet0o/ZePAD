...
from utils.feature_mask import create_feature_mask
...
def test(args, encoder, classifier, test_loader, data):
    ...
    with torch.no_grad():
        for counter, (x_batch, y_batch) in enumerate(tqdm(test_loader)):
            ...
            # 创建掩码并应用到特征上
            mask = create_feature_mask(downstream_input, args.mask_rate)
            masked_input = downstream_input * mask.float()
...
def fr_test(args, encoder, classifier, test_loader, uap, data):
    ...
    with torch.no_grad():
        for counter, (x_batch, y_batch) in enumerate(tqdm(test_loader)):
            ...
            c_downstream_input = c_h.view(c_h.size(0), -1)
            p_downstream_input = p_h.view(p_h.size(0), -1)
#             h = encoder(normalzie(args, x_batch))
#             downstream_input = h.view(h.size(0), -1)
#             # 创建掩码并应用到特征上
#             mask = create_feature_mask(downstream_input, args.mask_rate)
...
#             logits = classifier(masked_input)
            mask = create_feature_mask(c_downstream_input, args.mask_rate)
            c_masked_input = c_downstream_input*mask.float()
            p_masked_input = p_downstream_input*mask.float()
            # c_x_in = c_h.view(c_h.size(0), -1)
            # p_x_in = p_h.view(p_h.size(0), -1)

            c_logits = classifier(c_masked_input)
            p_logits = classifier(p_masked_input)
